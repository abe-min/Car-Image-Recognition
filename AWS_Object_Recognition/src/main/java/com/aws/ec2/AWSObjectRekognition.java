...
public class AWSObjectRekognition {
    public static void main(String[] args) throws IOException, JMSException, InterruptedException {
        ...
        try {
            ...
            // Get the messaging wrapped client
            AmazonSQSMessagingClientWrapper client = connection.getWrappedAmazonSQSClient();

            // Create an Amazon SQS FIFO queue named ImageRecognitionQue.fifo, if it doesn't already exist
            if (!client.queueExists("ImageRecognitionQue.fifo")) {
                Map<String, String> attributes = new HashMap<String, String>();
                attributes.put("FifoQueue", "true");
                attributes.put("ContentBasedDeduplication", "true");
                client.createQueue(new CreateQueueRequest().withQueueName("ImageRecognitionQue.fifo").withAttributes(attributes));
            }
            // Create the session
            Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
            // Create a queue identity and pass the queue name to the session
            Queue queue = session.createQueue("ImageRecognitionQue.fifo");

            // Create a producer for the 'ImageRecognitionQue'
                    ...
                    try {
                        ...
                        for (Label label : labels) {
                            if (label.getName().equals("Car") & label.getConfidence() > 90) {
                                System.out.print("Detected labels for the following:  " + photo + " => ");
